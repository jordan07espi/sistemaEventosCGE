...
$qr_x = 140;
$qr_y = $pdf->GetY();
$pdf->Image($pdf->generarQR($datosParticipante['cedula']), $qr_x, $qr_y, 50, 50, 'PNG');
$pdf->SetFont('Arial', '', 12);
...
$pdf->MultiCell(120, 8, iconv('UTF-8', 'ISO-8859-1//TRANSLIT//IGNORE', $textoParticipante), 0, 'L');
$pdf->Ln(25);

// --- SECCIÓN: DETALLES DEL BOLETO ---
$pdf->SetFont('Arial', 'B', 14);
$pdf->Cell(0, 10, 'DETALLES DEL BOLETO', 0, 1, 'L');
$pdf->SetFont('Arial', '', 12);
$pdf->Cell(95, 8, iconv('UTF-8', 'ISO-8859-1//TRANSLIT//IGNORE', "Categoría: " . $datosEntrada['nombre_entrada']), 0, 0, 'L');

// Lógica para mostrar "GRATUITO" o el precio
if ((float)$datosEntrada['precio'] === 0.00) {
    $textoPrecio = 'GRATUITO';
} else {
    $textoPrecio = '$' . number_format($datosEntrada['precio'], 2);
}
$pdf->Cell(95, 8, "Precio: " . $textoPrecio, 0, 1, 'R');
...
// --- 4. Salida del PDF ---
$pdf->Output('D', 'Boleto_' . str_replace(' ', '_', $datosEntrada['nombre_evento']) . '.pdf');
exit();
?>